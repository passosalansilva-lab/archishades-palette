...
export default function PixTransactions() {
  ...
  const loadTransactions = async () => {
    ...
    try {
      ...
      // Busca pedidos com payment_method = 'pix'
      let query = supabase
        .from('orders')
        .select('id, customer_name, customer_email, customer_phone, total, payment_status, stripe_payment_intent_id, created_at')
  ...
  const getPaymentProvider = (paymentId: string | null): string => {
    if (!paymentId) return 'Desconhecido';
    if (paymentId.startsWith('picpay_')) return 'PicPay';
    if (paymentId.startsWith('mp_') || /^\d+$/.test(paymentId)) return 'Mercado Pago';
    return 'PIX';
  };
  ...
  const exportTransactions = () => {
    ...
    const csv = [
      ['ID', 'Cliente', 'Email', 'Telefone', 'Valor', 'Status', 'Provedor', 'Data'].join(','),
      ...filteredTransactions.map(tx => [
        ...
        tx.total.toFixed(2),
        tx.payment_status,
        getPaymentProvider(tx.stripe_payment_intent_id),
    ...
    link.href = url;
    link.download = `transacoes-pix-${format(new Date(), 'yyyy-MM-dd')}.csv`;
    link.click();
  ...
  return (
    <DashboardLayout>
      <div className="max-w-7xl mx-auto space-y-6 animate-fade-in">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div>
            ...
            <p className="text-muted-foreground">
              Acompanhe todos os pagamentos realizados via PIX
            </p>
          </div>
        ...
        <Card>
          <CardHeader>
            <CardTitle className="text-lg">Histórico de Transações PIX</CardTitle>
            <CardDescription>
            ...
            {loading ? (
              ...
              <div className="text-center py-12">
                ...
                <p className="text-sm text-muted-foreground">
                  Ajuste os filtros ou aguarde novos pagamentos via PIX
                </p>
              </div>
            ) : (
              ...
              <div className="space-y-4">
                ...
                <div className="space-y-3">
                  ...
                  <div className="flex items-center justify-between py-2 border-b">
                    ...
                    <Badge variant="outline">
                      {getPaymentProvider(selectedTransaction.stripe_payment_intent_id)}
                    </Badge>
                  </div>
